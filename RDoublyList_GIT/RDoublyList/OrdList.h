...
Purpose: This file contains the specification for an ascending Ordered List Class. The list is implemented
		 ...
		 recursive counterparts. The class containts 9 public methods, 3 overloaded operators, 8 protected methods,
...
public:
	...
	OrdListClass(const OrdListClass& orig) -- Copy Constructor. Instantiates an OrdList object as a deep copy of orig.
	...
	int GetLength(); -- Returns # of items in the list
...
protected:
	...
	void rCopyConstructor(node* trav, node* origTrav, node* origtail); -- recursive copy constructor
...
public:
	...
	*/
	...
	/*
	Purpose: Similar to a Wrapper method, but for rCopyConstructor().
	Pre: List object exists.
	Post: rCopyConstructor() has been called.
	*/

	// ------------------------------------------------------------------------------------------------
	// |																							  |
	// |									Observer Predicators	(2)							(5)	  |
	// |																							  |
	// ------------------------------------------------------------------------------------------------
	...
	*/
	int GetLength();
	/*
	Purpose: return length of the list
	Pre: List object exists.
	Post: sends length of the list
	*/
	// ------------------------------------------------------------------------------------------------
	// |																							  |
	// |									Observer Summarizers	(2)							(12)  |
	// |																							  |
	// ------------------------------------------------------------------------------------------------
	...
	*/
	...
	// O(N + M)
	virtual OrdListClass operator +(/*in*/ const OrdListClass& otherList) const; // List to be added to applying object.
	/*
	Purpose: Combines rtOp and the applying object. Any items that appear in both lists only appear once in the returned list.
	Pre: Both objects exist.
	Post: The union of the two ordered lists will be returned. If the union of the two lists exceeds available memory, an exception will be thrown.
	*/

	// O(N + M)
	virtual OrdListClass operator -(/*in*/ const OrdListClass& rtOp) const; // List to be subtracted from applying object.
	/*
	Purpose: Subtracts rtOp from applying object, removing all elements that appear in both lists from the applying object.
	Pre: Both objects exist.
...
private:
	// PDM
	node* tail;	 // start of list
	int count; // count elements
	bool memFull; // is there memory

// ------------------------------------------------------------------------------------------------
// |																							  |
// |									Recursive (Wrapped) Methods		(8)				(23)	  |
// |																							  |
// ------------------------------------------------------------------------------------------------
...
protected:
	...
	Exceptions Thrown: OutOfMemException.
	*/
	// O(N)
	void rCopyConstructor(/*in*/node* trav,		  // tail of this list
						  /*in*/ node* origTrav,  // traverse for other list
						  /*in*/ node* origtail); // tail of other list
	/*
	Purpose: copies data into a new class object as a deep copy of orig.
	Pre: Both orig and applying object exist and tails have been sent
